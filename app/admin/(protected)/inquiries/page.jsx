...
// ── Mail Modal ──────────────────────────────────────────────
function MailModal({ inquiry, onClose, onSent }) {
  const dynamicSubject = inquiry.property_title
    ? `Your Inquiry – ${inquiry.property_title}`
    : 'Your Inquiry – Michael Leber Immobilien'

  const [subject,  setSubject]  = useState(dynamicSubject)
  ...
  const handleSend = async () => {
    ...
    setErrMsg('')
    try {
      const res = await adminApi.sendMail({ to: inquiry.email, toName: inquiry.name, subject, message, propertyTitle: inquiry.property_title || null })
      if (res.success) {
        setResult('ok')
        onSent?.()
      } else {
        setResult('err')
        setErrMsg(res.message || 'Failed to send email')
      }
    } catch (e) {
      setResult('err')
      setErrMsg(e.name === 'AbortError' ? 'Request timed out — check server' : e.message)
    }
    setSending(false)
  }
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: '#1e2a3a', borderRadius: '20px', width: '100%', maxWidth: '520px',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 24px', borderBottom: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.03)', borderRadius: '20px 20px 0 0' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ width: '34px', height: '34px', background: 'rgba(96,165,250,0.15)', border: '1px solid rgba(96,165,250,0.3)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <i className="fa fa-envelope" style={{ color: '#60a5fa', fontSize: '14px' }} />
            </div>
        ...
        <div style={{ padding: '24px' }}>
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: '#0f1623', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '10px', padding: '11px 14px' }}>
              <i className="fa fa-user-circle" style={{ color: '#475569', fontSize: '16px', flexShrink: 0 }} />
              <div>
                <p style={{ color: '#f1f5f9', fontSize: '14px', fontWeight: '600', margin: 0 }}>{inquiry.name}</p>
                <p style={{ color: '#60a5fa', fontSize: '13px', margin: 0 }}>{inquiry.email}</p>
              </div>
            </div>
          </div>
          ...
          {result === 'err' && (
            <div style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.35)', borderRadius: '10px', padding: '12px 16px', marginBottom: '16px', display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
              ...
              <p style={{ color: '#f87171', fontSize: '13px', fontWeight: '600', margin: 0 }}>{errMsg || 'Failed to send. Check mail config in .env'}</p>
            </div>
          )}

          {/* Actions */}
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
            <button onClick={onClose} style={{ padding: '10px 20px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', color: '#94a3b8', borderRadius: '10px', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>
              Cancel
            </button>
...
export default function Inquiries() {
  ...
  const fetchData = async () => {
    ...
    if (propRes.success) {
      const map = {}
      propRes.data.forEach(p => { map[p.id] = p })
      setPropMap(map)
    }
  }
  ...
  const selectRow = (inq) => {
    setSelected(inq)
    setPropDetail(inq.property_id ? propMap[inq.property_id] || null : null)
  }
  ...
  const handleDelete = async (id) => {
    ...
    if (selected?.id === id) { setSelected(null); setPropDetail(null) }
    setInquiries(prev => prev.filter(i => i.id !== id))
  }

  const filtered = inquiries.filter(inq => {
    const q = search.toLowerCase()
    const prop = inq.property_id ? propMap[inq.property_id] : null
    const matchSearch = !q || inq.name?.toLowerCase().includes(q) || inq.email?.toLowerCase().includes(q) || prop?.title?.toLowerCase().includes(q)
  ...
  return (
    <>
      {mailTarget && <MailModal inquiry={mailTarget} onClose={() => setMailTarget(null)} onSent={() => updateStatus(mailTarget.id, 'Replied')} />}
      ...
      <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', gap: '10px', marginBottom: '16px', flexWrap: 'wrap', alignItems: 'center' }}>
            <div style={{ position: 'relative', flex: 1, minWidth: '200px' }}>
              ...
              <input type="text" placeholder="Search by name, email or property..." value={search}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)', background: '#141824' }}>
                  {['#', 'Name', 'Email', 'Property', 'Status', 'Actions'].map(h => (
                ...
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan="6" style={{ padding: '60px', textAlign: 'center', color: '#64748b', fontSize: '15px' }}>
                      ...
                      {search || statusFilter !== 'All' ? 'No matching inquiries found' : 'No inquiries yet'}
                    </td>
                  </tr>
                ) : paginated.map((inq, i) => {
                  const linkedProp = inq.property_id ? propMap[inq.property_id] : null
                  return (
                    <tr key={inq.id}
                      onClick={() => selectRow(inq)}
                      style={{
                        borderBottom: '1px solid rgba(255,255,255,0.06)',
                        cursor: 'pointer',
                        background: selected?.id === inq.id ? 'rgba(37,99,235,0.12)' : 'transparent',
                        borderLeft: selected?.id === inq.id ? '3px solid #2563eb' : '3px solid transparent',
                        transition: 'all 0.15s',
                      }}
                      onMouseEnter={e => { if (selected?.id !== inq.id) e.currentTarget.style.background = 'rgba(255,255,255,0.04)' }}
                    ...
                    >
                      ...
                      <td style={tdStyle({ color: '#94a3b8', fontSize: '13px' })}>{inq.email}</td>
                      <td style={tdStyle()}>
                        {linkedProp
                          ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', background: 'rgba(96,165,250,0.12)', color: '#60a5fa', border: '1px solid rgba(96,165,250,0.25)', padding: '4px 10px', borderRadius: '6px', fontSize: '12px', fontWeight: '600', maxWidth: '160px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                              <i className="fa fa-building" style={{ fontSize: '11px', flexShrink: 0 }} />
                              {linkedProp.title}
                            </span>
                      ...
                      <td style={tdStyle()}>
                        <span style={{ background: statusColor[inq.status] + '20', color: statusColor[inq.status], border: `1px solid ${statusColor[inq.status]}50`, padding: '5px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: '700' }}>
                          {inq.status}
                        </span>
                      </td>
                      <td style={tdStyle()}>
                        <div style={{ display: 'flex', gap: '6px' }}>
                          <button onClick={e => { e.stopPropagation(); setMailTarget(inq) }} title="Send Email"
                          ...
                          <button onClick={e => { e.stopPropagation(); handleDelete(inq.id) }} title="Delete"
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '16px', flexWrap: 'wrap', gap: '12px' }}>
            <p style={{ color: '#64748b', margin: 0, fontSize: '13px' }}>{filtered.length} / {inquiries.length} inquiries</p>
        ...
        {selected && (
          <div style={{ width: '360px', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ background: '#1a1f2e', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.1)', padding: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '18px' }}>
                <h4 style={{ color: '#f1f5f9', margin: 0, fontSize: '15px', fontWeight: '700' }}>Inquiry Detail</h4>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button onClick={() => setMailTarget(selected)} title="Send Email" style={{ background: 'rgba(96,165,250,0.15)', border: '1px solid rgba(96,165,250,0.3)', color: '#60a5fa', width: '30px', height: '30px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <i className="fa fa-paper-plane" />
                  </button>
                  <button onClick={() => { setSelected(null); setPropDetail(null) }} style={{ background: 'rgba(255,255,255,0.08)', border: 'none', color: '#94a3b8', width: '30px', height: '30px', borderRadius: '8px', cursor: 'pointer', fontSize: '18px', fontWeight: '700', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '16px' }}>
                <div>
                  ...
                  <p style={{ color: '#f1f5f9', fontSize: '14px', fontWeight: '600', margin: 0 }}>
                    <i className="fa fa-user" style={{ color: '#475569', marginRight: '8px', width: '14px' }} />{selected.name}
                  </p>
                </div>
                <div>
                  <Label>Email</Label>
                  <p style={{ color: '#94a3b8', fontSize: '13px', margin: 0 }}>
                    <i className="fa fa-envelope" style={{ color: '#475569', marginRight: '8px', width: '14px' }} />{selected.email}
                  </p>
                </div>
                {selected.phone && (
                  <div>
                    <Label>Phone</Label>
                    <p style={{ color: '#94a3b8', fontSize: '13px', margin: 0 }}>
                      <i className="fa fa-phone" style={{ color: '#475569', marginRight: '8px', width: '14px' }} />{selected.phone}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div style={{ marginBottom: '14px' }}>
                <Label>Status</Label>
                <select value={selected.status} onChange={e => updateStatus(selected.id, e.target.value)} style={{ width: '100%', marginTop: '6px', padding: '9px 12px', borderRadius: '10px', background: statusColor[selected.status] + '20', color: statusColor[selected.status], border: `1px solid ${statusColor[selected.status]}50`, fontSize: '13px', fontWeight: '700', cursor: 'pointer', outline: 'none' }}>
                  <option value="New">New</option>
                  <option value="Replied">Replied</option>
                  <option value="Closed">Closed</option>
                </select>
            ...
            {propDetail && (
              <div style={{ background: '#1a1f2e', borderRadius: '16px', border: '1px solid rgba(96,165,250,0.2)', overflow: 'hidden' }}>
                <div style={{ padding: '12px 18px', background: 'rgba(96,165,250,0.08)', borderBottom: '1px solid rgba(96,165,250,0.15)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <i className="fa fa-building" style={{ color: '#60a5fa', fontSize: '13px' }} />
                  <span style={{ color: '#60a5fa', fontSize: '12px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.6px' }}>Linked Property</span>
                </div>
                {propDetail.image && (
                  <img src={`${API}${propDetail.image}`} alt={propDetail.title} style={{ width: '100%', height: '160px', objectFit: 'cover', display: 'block' }} />
                )}
                <div style={{ padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  <p style={{ color: '#f1f5f9', fontSize: '15px', fontWeight: '700', margin: 0 }}>{propDetail.title}</p>
                  <p style={{ color: '#94a3b8', fontSize: '13px', margin: 0 }}>
                    <i className="fa fa-map-marker" style={{ color: '#60a5fa', marginRight: '6px' }} />{propDetail.location}
                  </p>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <p style={{ color: '#34d399', fontSize: '16px', fontWeight: '700', margin: 0 }}>€ {Number(propDetail.price).toLocaleString()}</p>
                    <span style={{ background: 'rgba(52,211,153,0.15)', color: '#34d399', border: '1px solid rgba(52,211,153,0.35)', padding: '4px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '700' }}>{propDetail.status}</span>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                    {[
                      { label: 'Size',      value: propDetail.size      ? `${propDetail.size} m²` : '—' },
                      { label: 'Rooms',     value: propDetail.rooms     || '—' },
                      { label: 'Bedrooms',  value: propDetail.bedrooms  || '—' },
                      { label: 'Bathrooms', value: propDetail.bathrooms || '—' },
                    ].map(({ label, value }) => (
                      <div key={label} style={{ background: '#0f1623', borderRadius: '8px', padding: '8px 10px' }}>
                        <p style={{ color: '#475569', fontSize: '10px', fontWeight: '700', textTransform: 'uppercase', margin: '0 0 2px' }}>{label}</p>
                        <p style={{ color: '#cbd5e1', fontSize: '13px', fontWeight: '600', margin: 0 }}>{value}</p>
                      </div>
                    ))}
                  </div>
                </div>
