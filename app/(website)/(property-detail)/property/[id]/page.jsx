...
export default function PropertyDetail() {
  ...
  // Re-initialize jQuery plugins after SPA navigation (scripts run once on first load only)
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // Isotope masonry filter for gallery
      ...
      // Scroll animations
      if (jq.fn && jq.fn.scrolla) {
        jq('.animate').scrolla({ mobile: false, once: true })
      }

      // Magnific popup lightbox for gallery
  ...
  const handleSubmit = async (e) => {
    ...
    setSending(true)
    try {
      await websiteApi.submitInquiry({ ...form, propertyTitle: property?.title })
    } catch (_) {}
    setSent(true)
    setSending(false)
  ...
  return (
    <div className="page-wraper">
      ...
      >
        ...
        {/* Hero image — ken burns zoom-out (matches Revolution Slider data-scalestart="130" data-scaleend="100" data-duration="20000") */}
        <style>{`
          @keyframes kenBurns {
          ...
          .tp-bannertimer {
            ...
            height: 10px; background: rgba(255,255,255,0.25);
            z-index: 10;
            animation: timerBar 20s linear forwards;
          }
        `}</style>
      ...
      {!loading && !property && (
        <section className="section-padding text-center">
          <div className="container">
            ...
            <Link href="/property"><button className="btn btn1">{tr.backBtn}</button></Link>
          </div>
        </section>
      )}
              ...
              {property.description && (
                <div className="col-lg-10 col-md-12 mx-auto">
                  <div className="text-center pera2">
                    ...
                    <a href="#anfragen">
                      <button type="button" className="btn btn1">{tr.exposeBtn}</button>
                    </a>
                  </div>
                </div>
              )}
                ...
                <div className="row">
                  {gallery.length > 0 ? gallery.map((img, i) => (
                    <div key={img.id} className={`masonry-item ${i % 2 === 0 ? 'cat-filter-1' : 'cat-filter-2'} col-xl-8 col-lg-8 col-md-8 col-12 mx-auto`}>
                      <div className="wt-box">
                        <div className="wt-thum-bx">
                          <img src={`${API}${img.image}`} alt="gallery" />
                        </div>
                      </div>
                    </div>
                  )) : (
                    <div className="masonry-item cat-filter-1 col-xl-8 col-lg-8 col-md-8 col-12 mx-auto">
                      <div className="wt-box">
                        <div className="wt-thum-bx">
                          <img src={heroImg} alt={property.title} />
                        </div>
                      </div>
                    </div>
                  )}
                      ...
                      <div className="row">
                        <div className="col-lg-6 col-md-6">
                          <div className="form-group">
                            <input type="text" className="form-control" placeholder={tr.namePh} required
                              value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />
                          </div>
                        </div>
                        <div className="col-lg-6 col-md-6">
                          <div className="form-group">
                            <input type="tel" className="form-control" placeholder={tr.phonePh}
                              value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} />
                          </div>
                        </div>
                        <div className="col-lg-12 col-md-12">
                          <div className="form-group">
                            <input type="email" className="form-control" placeholder={tr.emailPh} required
      ...
      <button className="scroltop">
        <span className="iconmoon-house relative" id="btn-vibrate"></span>Top
      </button>
